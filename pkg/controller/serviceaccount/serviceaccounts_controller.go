...
func NewServiceAccountsController(saInformer kcpcorev1informers.ServiceAccountClusterInformer, nsInformer kcpcorev1informers.NamespaceClusterInformer, cl kcpkubernetesclientset.ClusterInterface, options ServiceAccountsControllerOptions) (*ServiceAccountsController, error) {
	...
	if cl != nil && cl.Cluster(logicalcluster.Name("fake").Path()).CoreV1().RESTClient().GetRateLimiter() != nil {
		if err := ratelimiter.RegisterMetricAndTrackRateLimiterUsage("serviceaccount_controller", cl.Cluster(logicalcluster.Name("fake").Path()).CoreV1().RESTClient().GetRateLimiter()); err != nil {
			return nil, err
		}
	}
